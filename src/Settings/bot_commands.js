...
exports.btnRecieveCommands = async (bot, data, command, chat_id, message, sentMessage, moderator, admin, group, recMessage) => {
    if (data === command || data === `${command}@InsuranceUZBOT`) {
        bot.sendMessage(chat_id, message)
            .then(bot.on('message', async msg => {
                ...
                if (msg.document || msg.photo) {
                    ...
                    await bot.sendMessage(chat_id, sentMessage)
                } else {
                    await bot.sendMessage(chat_id, `It's not a file`)
                }
